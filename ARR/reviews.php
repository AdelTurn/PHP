<?php 
    ...
    try {
        $sql = "SELECT review_id, review_name, review_body, review_img, review_rating, review_date FROM arr_reviews ORDER BY review_id DESC;";
...
<div class="border-bottom site-header sticky-top bg-white">
    ...
    <div class="bodyCon">
        <div class="reviewsCon">
            <h1>Adel's Restaurant Reviews</h1>
        <?php 
        foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $result) {
            $reviewId = $result["review_id"];
        ?>
            <div class="reviewBlock">
                <h2><?php echo $result["review_name"]; ?></h2>
                <?php 
                
                $date=date_create($result["review_date"]);
                
                ?>
                <h5>Date: <?php echo date_format($date, "m/d/Y"); ?></h5>
                <img src="img/<?php echo $result["review_img"]; ?>" alt="">
                <p><?php echo $result["review_body"]; ?></p>
                <h2>Rating: <?php echo $result["review_rating"]; ?>/10</h2>
                <?php 
                    if(isset($_SESSION["loggedIn"])) {
                ?>
                <div class="buttonBlock">
                    <a href="delete.php?reviewId=<?php echo $reviewId; ?>">Delete</a>
                </div>
                <?php
                }
                ?>
